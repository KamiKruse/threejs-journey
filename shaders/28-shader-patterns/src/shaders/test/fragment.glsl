...
{   
    ...
    //Pattern 12
    //Bruno's Method - Preferred method 
    // float strength = step(0.8, mod(vUv.x * 10.0, 1.0));
    // strength *= step(0.8, mod(vUv.y * 10.0, 1.0));
    
    // gl_FragColor = vec4(vec3(strength),1.0);

    //My Method - mine produces same result but less efficient
    ...
    //Pattern 20
    // float strength = max(abs(vUv.x - 0.5),abs(vUv.y - 0.5));
    // Same size square but the frame is inside -  my mistake of not reading the requirement
    // strength = step(0.4, strength);
    // Simons implementation. Requirement was the same pattern but much smaller square
    // float outerBox = step(0.2, max(abs(vUv.x - 0.5),abs(vUv.y - 0.5)));
    // float innerSquare = 1.0 - step(0.25, max(abs(vUv.x - 0.5),abs(vUv.y - 0.5)));
    // And make the frame within that square
    // float finalStrength = outerBox * innerSquare;
    ...
    //Pattern 25
    // float strength = random(floor((vUv + vUv.x/2.0) * 10.0)/10.0);
    ...
    //Pattern 30
    // float strength = 0.015 / distance(vec2(vUv.x/10.0 + 0.45 , vUv.y/2.0 + 0.25), vec2(0.5)); 

    //Pattern 31
//    float strength1 = 0.01 / distance(vec2(vUv.x/10.0 + 0.45 , vUv.y/2.0 + 0.25), vec2(0.5));
//    float strength2 = 0.01 / distance(vec2(vUv.x/2.0 + 0.25 , vUv.y/10.0 + 0.45), vec2(0.5));
//    float strength = strength1 * strength2;

    //Pattern 32
//     vec2 rotatedUv = rotate(vUv, PI * 0.25, vec2(0.5));
//    float strength1 = 0.01 / distance(vec2(rotatedUv.x/10.0 + 0.45 , rotatedUv.y/2.0 + 0.25), vec2(0.5));
//    float strength2 = 0.01 / distance(vec2(rotatedUv.x/2.0 + 0.25 , rotatedUv.y/10.0 + 0.45), vec2(0.5));
//    float strength = strength1 * strength2;
...
//    angle /= 2.0 * PI;
//    angle += 0.5;
// // angle = step(mod(angle * 20.0, 1.0), 0.5);
...
//    float radius = 0.15 + sinsoid * 0.02;
...
//Clamp the Strength so that it doesn't go above or below the color especially in a mix
